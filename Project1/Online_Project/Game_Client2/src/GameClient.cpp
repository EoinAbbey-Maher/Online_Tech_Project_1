...
bool GameClient::ProcessPacketType(PacketType packetType)
{
	switch (packetType)
	{
	case PacketType::ChatMessage: //If PacketType is a chat message PacketType
	{
		...
		m_recievedMessage = Message;
		//std::cout << m_recievedMessage << std::endl; //Display the message to the user
		break;
	}
...
void GameClient::ClientThread(GameClient& client)
{
...
bool GameClient::RequestFile(const std::string& fileName)
{
	if (m_file.m_transferInProgress)
	{
		std::cout << "ERROR: Function(GameClient::RequestFile) - File Transfer already in progress.\n";
		return false;
	}
	...
	{
		std::cout << "ERROR: Function(GameClient::RequestFile) - Unable to open file: " << fileName << " for writing.\n";
		return false;
	}
...
void GameClient::PacketSenderThread(GameClient& client) //Thread for all outgoing packets
{
	while (true)
	{
		if (client.m_terminateThreads == true)
